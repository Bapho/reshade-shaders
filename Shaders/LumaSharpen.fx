...
float3 LumaSharpenPass(float4 position : SV_Position, float2 tex : TEXCOORD0) : SV_Target
{
	...
	if (inversedAttribution){
		float3 revert_sharp_strength = (1.01 / (abs(sharp) + 0.01)) / 30.0;
	...
	if (show_sharpen)
	{
		...
		float3 normal = tex2D(ReShade::BackBuffer, tex).rgb;
		
		if (((normal.r > 0.0 && normal.r < 1.0) 
				|| (normal.g > 0.0 && normal.g < 1.0)
				|| (normal.b > 0.0 && normal.b < 1.0))
					&& ((outputcolor.r >= 1.0 && outputcolor.g >= 1.0 && outputcolor.b >= 1.0)
						|| (outputcolor.r <= 0.0 && outputcolor.g <= 0.0 && outputcolor.b <= 0.0))){
